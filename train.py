...
if __name__ == "__main__":
    ...
    parser.add_argument("--choose_features", default=['wind_speed_80m(m/s)','relative_humidity_2m(%)'], type=list)
    ...
    parser.add_argument("--input_config", default="config/base.yaml", type=str)  # input config file
...
"""
    ...
    train_data = TestDataset(args, config)
    # val_data = TestDataset(args, config)
    val_data = TrainDataset(args, config)
    ...
    model = TimeSeriesRWKV(args, config)
    if args.model_path:
    ...
    val_loader = DataLoader(val_data, shuffle=False, pin_memory=True, batch_size=1, num_workers=1, 
                           persistent_workers=False, drop_last=True)
